...
class LatentVariableEstimator:
    def __init__(
        ...
        if self._anomaly_detection:
            self._latent_variable_loss = AnomalyLoss(
                **self._params["loss_anomaly"],
                log_enabled=self._params["general"]["log_confidence"],
                log_folder=self._params["general"]["model_path"],
            )
            self._latent_variable_loss.to(self._device)

        else:
            self._latent_variable_loss = TraversabilityLoss(
                **self._params["loss"],
                model=self._model,
                log_enabled=self._params["general"]["log_confidence"],
                log_folder=self._params["general"]["model_path"],
    ...
    def reset(self):
        print("[WARNING] Resetting the latent variable estimator is not fully tested")
    ...
    def add_supervision_node(self, pnode: SupervisionNode):
        ...
        if not success:
            # Update latent variable of latest node
            if last_pnode is not None:
                last_pnode.update_latent_variable(pnode.latent_variable, pnode.latent_variable_var)
            return False
            ...
            mask = mask * pnode.latent_variable
    ...
    def save(self, mission_path: str, filename: str):
        """Saves a pickled file of the LatentVariableEstimator class
    ...
    def load(cls, file_path: str, device="cpu"):
        """Loads pickled file and creates an instance of LatentVariableEstimator,
            ...
            torch.save(
                {
                    ...
                    "latent_variable_loss_state_dict": self._latent_variable_loss.state_dict(),
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        with self._learning_lock:
            ...
            self._latent_variable_loss.load_state_dict(checkpoint["latent_variable_loss_state_dict"])
        ...
        if num_valid_nodes > self._min_samples_for_training:
            ...
            if graph is not None:
                with self._learning_lock:
                    ...
                    self._loss, loss_aux, trav = self._latent_variable_loss(
